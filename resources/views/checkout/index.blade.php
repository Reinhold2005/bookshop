...
<div class="max-w-6xl mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <form id="checkoutForm" method="POST" action="{{ route('checkout.session') }}">
                @csrf
                ...
                <div class="bg-white rounded-lg shadow p-6 mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <label class="border rounded-lg p-4 cursor-pointer hover:border-blue-500 transition delivery-option" data-price="{{ $option['price'] }}" data-days="{{ $option['days'] }}">
                            <input type="radio" name="delivery_method" value="{{ $key }}" required class="mr-2">
                            <i class="fas {{ $option['icon'] }} text-blue-500 mr-2"></i>
                            <strong>{{ $option['name'] }}</strong>
                            <div class="text-sm text-gray-600 mt-1">+N${{ number_format($option['price'], 2) }}</div>
                            <div class="text-xs text-green-600 mt-1">
                                <i class="fas fa-clock"></i> Est: {{ $option['days'] }}
                            </div>
                        </label>
                        @endforeach
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-6 sticky top-4">
                ...
                <div class="border-b pb-3 mb-3">
                    ...
                    <div class="flex justify-between mb-2 text-sm">
                        ...
                        <span>N${{ number_format($item->book->price * $item->quantity, 2) }}</span>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex justify-between text-lg font-bold border-t pt-2 mt-2">
                        <span>Total:</span>
                        <span id="totalAmount">N${{ number_format($total, 2) }}</span>
                    </div>
                </div>
