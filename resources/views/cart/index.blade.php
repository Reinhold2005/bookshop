                    ...
                    <div class="flex flex-col md:flex-row">
                        ...
                        <div class="flex-1 p-4">
                            <div class="flex flex-col md:flex-row justify-between">
                                <div class="flex-1">
                                    <h3 class="font-bold text-xl mb-1">{{ $item->book->title }}</h3>
                                    <p class="text-gray-600 text-sm mb-1">by {{ $item->book->author }}</p>
                                    <p class="text-gray-500 text-xs mb-2">{{ $item->book->category }}</p>
                                    <p class="text-green-600 font-bold text-2xl mb-3">${{ number_format($item->book->price, 2) }}</p>
                                ...
                                <div class="mt-4 md:mt-0 md:text-right">
                                    <div class="flex items-center gap-2 mb-3">
                                        ...
                                        <div class="flex items-center border rounded-lg overflow-hidden">
                                            ...
                                            <span id="qty-{{ $item->id }}" class="w-10 text-center font-semibold">{{ $item->quantity }}</span>
                                            <button onclick="updateQuantity({{ $item->id }}, {{ $item->quantity + 1 }})" 
                                                    class="w-8 h-8 bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition"
                                    ...
                                    <p class="text-gray-600 text-sm mb-2">
                                        Subtotal: <span class="font-bold text-green-600" id="subtotal-{{ $item->id }}">
                                            N${{ number_format($item->book->price * $item->quantity, 2) }}
                                        </span>
                                    </p>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-lg shadow-lg p-6 sticky top-4">
                    ...
                    <div class="space-y-3 border-b pb-4">
                        @php $total = 0; @endphp
                        @foreach($cartItems as $item)
                            @php $itemTotal = $item->book->price * $item->quantity; $total += $itemTotal; @endphp
                            <div class="flex justify-between text-sm">
                                <span>{{ $item->book->title }} (x{{ $item->quantity }})</span>
                                <span>N${{ number_format($itemTotal, 2) }}</span>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="space-y-2 mt-4">
                        <div class="flex justify-between">
                            <span>Subtotal:</span>
                            <span class="font-semibold" id="cartTotal">N${{ number_format($total, 2) }}</span>
                        </div>
                    ...
                    <div class="border-t pt-4 mt-4">
                        <div class="flex justify-between text-lg font-bold">
                            <span>Total:</span>
                            <span class="text-green-600" id="grandTotal">N${{ number_format($total, 2) }}</span>
                        </div>
                    </div>
